...
Token *getToken(void)
{
  ...
  case 2: //TODO (Skip blanks)
    readChar();
    state = 0;
    return getToken();
  case 3: // TODO Recognize Identifiers and keywords
    ln = lineNo;
    cn = colNo;
    int count = 1;
  ...
  case 42:
    ...
    cn = colNo;
    readChar();
    if (ln != lineNo) return makeToken(SB_RPAR, ln, cn);
